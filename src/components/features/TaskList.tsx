...
import { Play, Pause, Check, Trash2, RotateCcw, ChevronDown, Search, X, AlertCircle } from 'lucide-react';

// Tab types
type ActiveTab = EisenhowerQuadrant | 'overdue';
type DoFirstTab = 'pending' | 'paused' | 'done';
...
export function TaskList() {
  ...
  const tabTasks = useMemo(() => {
    ...
    // ✅ Tab LÀM NGAY: Loại bỏ các việc quá hạn (chúng chỉ hiển thị ở tab QUÁ HẠN)
    if (activeTab === 'do_first') {
      return tasks.filter(t => t.quadrant === 'do_first' && !isTaskOverdue(t));
    }
